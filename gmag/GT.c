// NJP Oct 8 2018 attempt to get this to compile
//  copy of GBOMagUseBeagleboneTime.c
//
// =====================================================================
// GBOMag_v3.c
...
char inifilenam[255] = "/home/pi/code/gmag/gbo.ini";
...
int main (int argc, char *argv[])
{
   ...
   ***************************************************/
   ...
   // Use BeagleboneTime instead of GPS Time 04/07/2016 begin 
   // Now we are using Beaglebone minute and second time instead of GPS min and second time
   gpsmin = tm_ptr->tm_min;
   gpssec = tm_ptr->tm_sec;
   // Use BeagleboneTime 04/07/2016 end
   gpsstat = 13;
   /**********************
   * Read in "ini" file *
   **********************/
   (void) GtIni (inifilenam);
   ...
   *******************************/
   ...
   if (magdes == -1)
   {
      ...
      printf("Unable to open mag device %s\n",MagDevice);
      exit(-1);
   }
   else
   {
...
passbuf[0] = '\0';
   ...
   FILE* fp;
   fp = fopen("/home/pi/code/gmag/checkusb.txt", "a");
   while (TRUE)
   {
      if (syssig != 0)
      {
...
int TMsg ()
{
   ...
   ****************************************************/
   ...
   if (nread == 3)
   {
	 ...
	 K2[index] * DTemp * DTemp +
	 K3[index] * DTemp * DTemp * DTemp +
	 K4[index] * DTemp * DTemp * DTemp * DTemp +
	 K5[index] * DTemp * DTemp * DTemp * DTemp * DTemp;
      ...
      if (nread == 10)
	   {
		 ...
		 // Use BeagleboneTime 04/07/2016 begin
		 // Now we are using the Beaglebone hour, min, and second instead of the time obtained by the GPS
         /*Hr = SOD / 3600;
         Min = (SOD - Hr * 3600) / 60;
...
int HKD (int Hr, int Min, int Flag)
{
   ...
   gpshr = tm_ptr->tm_hour;
   gpsmin = tm_ptr->tm_min;
   gpssec = tm_ptr->tm_sec; // Added by William Tam
   gpsstat = 13;
   mode_t file_permissions = S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH;
