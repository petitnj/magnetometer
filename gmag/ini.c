...
#include "ini.h" //adding header include so that ini.h will work in standard way
//including necessary standard libraries so that ini.c will not depend on GBOMag.c(pcruce 2013-02-06)
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
 ...
 ***************************************/
char
FirstLetterIsOpenBracket (int line, IniFile * iniFile)
{
 ...
 ******************************/
char
ContainsCloseBracket (int line, IniFile * iniFile)
{
  int length = strlen (iniFile->inifile[line]);
  int i;
 ...
 *********************************************************/
void
CountNumberOfSections (IniFile * iniFile)
{
  ...
  iniFile->section_pos =
    (long *) malloc (sizeof (long) * iniFile->number_of_sections);
  memcpy (iniFile->section_pos, section_lines,
	  iniFile->number_of_sections * sizeof (long));
}
 ...
 *********************/
void
GetSectionNames (IniFile * iniFile)
{
  int length;
  int i, j;
  char *p;
 ...
 *********************************/
char
HasEntryName (IniEntry * entry, char *textline)
{
 ...
 **************************************/
char
ContainsEqualsSign (IniEntry * entry, char *textline)
{
  int length = strlen (textline);
  int i, j;
 ...
 ******************************************/
void
LastLetterBeforeEquals (IniEntry * entry, char *textline)
{
  int i;
  entry->last_letter = -1;
 ...
 ************************************************/
void
AddEntryName (IniEntry * entry, char *textline, int entry_number)
{
  int length;
  char *p;
  ...
  entry->entry_names[entry_number] =
 ...
 ******************************************/
char
FirstLetterAfterEquals (IniEntry * entry, char *textline)
{
  int i;
  int length = strlen (textline);
 ...
 *****************************************/
void
LastLetterAfterEquals (IniEntry * entry, char *textline)
{
  int i;
  int length = strlen (textline);
 ...
 ****************************************/
void
RemoveTrailingSpaces (IniEntry * entry, char *textline)
{
  int i, j;
 ...
 ***************************************************/
void
AddEntryValue (IniEntry * entry, char *textline, int entry_number)
{
  int length;
  char *p;
  if (FirstLetterAfterEquals (entry, textline))
    {
      ...
      entry->entry_values[entry_number] =
	(char *) malloc (sizeof (char) * (length + 1));
 ...
 ***********************************************************************/
void
ReadIniEntry (IniFile * iniFile, int pos, int first_line, int last_line)
{
  ...
  for (i = 0; i < number_of_lines; i++)
    {
      if (HasEntryName
	  (iniFile->entries[pos], iniFile->inifile[first_line + i]))
	{
  ...
  iniFile->entries[pos]->entry_names =
    (char **) malloc (sizeof (char *) *
		      iniFile->entries[pos]->number_of_entries);
  iniFile->entries[pos]->entry_values =
    (char **) malloc (sizeof (char *) *
		      iniFile->entries[pos]->number_of_entries);
  ...
  for (i = 0; i < number_of_lines; i++)
	  ...
	  (iniFile->entries[pos], iniFile->inifile[first_line + i]))
	{
	  if (ContainsEqualsSign
	      (iniFile->entries[pos], iniFile->inifile[first_line + i]))
	    {
	      AddEntryName (iniFile->entries[pos],
			    iniFile->inifile[first_line + i], j);
	      AddEntryValue (iniFile->entries[pos],
			     iniFile->inifile[first_line + i], j);
	      ++j;
	    }
	}
 ...
 **************************************************************/
char
Ini_ReadIniFile (char *filename, IniFile * iniFile)
{
  FILE *file;
  ...
  iniFile->inifile =
    (char **) malloc (sizeof (char *) * iniFile->number_of_lines);
  ...
  iniFile->section_names =
    (char **) malloc (sizeof (char *) * iniFile->number_of_sections);
  GetSectionNames (iniFile);
  iniFile->entries =
    (IniEntry **) malloc (sizeof (IniEntry *) * iniFile->number_of_sections);
  ...
  for (i = 0; i < iniFile->number_of_sections - 1; i++)
    {
      ...
      ReadIniEntry (iniFile, i, iniFile->section_pos[i],
		    iniFile->section_pos[i + 1]);
    }
  iniFile->entries[iniFile->number_of_sections - 1] =
    (IniEntry *) malloc (sizeof (IniEntry));
  ReadIniEntry (iniFile, iniFile->number_of_sections - 1,
		iniFile->section_pos[iniFile->number_of_sections - 1],
		iniFile->number_of_lines);
  fclose (file);
 ...
 ******************************/
int
FindSection (IniFile * iniFile, char *section)
{
  int i;
  for (i = 0; i < iniFile->number_of_sections; i++)
    {
      if (!strcmp (section, iniFile->section_names[i]))
	return i;
    }
  return -1;
}
 ...
 **************************/
int
FindEntry (IniEntry * e, char *entry)
{
  int i;
  for (i = 0; i < e->number_of_entries; i++)
    {
      if (!strcmp (entry, e->entry_names[i]))
	return i;
    }
  return -1;
}
 ...
 ***************************/
char *
EntryValue (IniEntry * e, char *entry)
{
  int entry_number;
  ...
  /* otherwise, there was a valid entry name, so return the value */
  return e->entry_values[entry_number];
}

int
NumberOfEntries (IniEntry * e)
{
  return e->number_of_entries;
}

char **
SectionNames (IniFile * iniFile)
{
  ...
  /* NumberOfSections(); */
int
NumberOfSections (IniFile * iniFile)
{
  return iniFile->number_of_sections;
}

char **
EntryNames (IniEntry * e)
{
  return e->entry_names;
}

void
Ini_FreeIniEntry (IniEntry * e)
{
  int i;
  for (i = 0; i < e->number_of_entries; i++)
    {
 ...
 ***********************************************************/
char **
Ini_SectionEntries (IniFile * iniFile, char *section)
{
 ...
 ***********************************************************/
int
Ini_NumberOfEntries (IniFile * iniFile, char *section)
{
  int s_number = FindSection (iniFile, section);
  if (s_number == -1)
    return -1;
 ...
 ***************************************************************/
char *
Ini_GetValue (IniFile * iniFile, char *section, char *entry)
{
  int s_number;
 ...
 ***************************************************/
void
Ini_FreeIniFile (IniFile * iniFile)
{
  int i;

  /* only want to do this if we actually allocated 
     stuff */
